...
const UpdateTicketService = async ({
  ...
  try {
    ...
    if (oldStatus === "closed") {
      await CheckContactOpenTickets(userId, ticket.contact.id, ticket.whatsappId);
      chatbot = null;
      queueOptionId = null;
    }
    ...
    if (status !== "campaign" && oldQueueId !== queueId && !isNil(oldQueueId) && !isNil(queueId)) {
    ...
    if (["pending", "open", "campaign"].indexOf(status) > -1) {
      const ttUpdate = {
        whatsappId: ticket.whatsappId,
        userId: ["open", "campaign"].indexOf(status) > -1 ? ticket.userId : null,
        startedAt: ["open", "campaign"].indexOf(status) > -1 ? moment().toDate() : null,
      };

      if (["open", "campaign"].indexOf(status) > -1) {
      ...
      ticketTraking.update(ttUpdate, seqOptions);
